...
## Description
This repository contains the software stack for completing the navigation and science missions of International Space Drone Competition 2025 (ISDC 2025) held at BITS Pilani, Goa Campus on 2nd February 2025. The team placed 4th overall and 1st amongst fully autonomous drones.
...
```
3. Download models (if using image\_analysis, depth\_estimator or automatic orange cone detection packages)
```bash
./download_models.sh 
```
...
4. ``cone_pixel``:
Detects pixel coordinates of cones, that are predicted using either a YOLO model(only orange cones) or manually, depending on the mode set.
5. ``cone_gps``:
